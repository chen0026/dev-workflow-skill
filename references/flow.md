...
`run` 输出初始护栏、`flow_reason`、`pre_code_gate`、`code_allowed` 和下一步动作；完成前运行 `verify` 和 `check`。这些输出不替代硬门禁和人工审核。
...
- 不创建 PRD / REQ。
- 默认不创建 TASK / BUG / ACC。
- 文档新增数量为 0；如果用户明确要求留痕，最多创建或更新 1 个 TASK。
- 不读完整历史文档。
- 简单验证和自查。
- 只在用户要求、变更需要长期追溯、或已有对应主记录时，创建或更新一个 TASK。
