...
注释是维护入口，不是代码翻译。新增或修改业务代码时，默认给关键逻辑补简短中文注释。

- quick：只有不注释会影响理解时，补 1 行短注释。
- standard：新增 / 修改的关键业务逻辑、复杂条件、状态变化、权限、金额、订单、文件、目录、数据一致性必须补注释。
- strict：关键规则注释应关联 REQ、验收项或业务规则。
- 注释说明“这段代码负责什么、为什么这样处理、边界或限制是什么”，不要解释显而易见的语法。
- 禁止逐行翻译代码、空泛注释和长背景。长背景写进 ACTIVE / REQ / history。
- 完成前自查：关键逻辑是否缺注释；已有注释是否过时；是否存在无意义注释。
